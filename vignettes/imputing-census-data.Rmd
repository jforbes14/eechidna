...
load("/Users/Jeremy/Documents/R/eechidna/data/abs2011.rda")
...
The Australian Electoral Commission shifts the electoral boundaries regularly, so the electoral divisions in place in the 2013 election may not match those in 2016, nor those in 2011. To obtain Census information for each electoral division, we cannot directly interpolate between neighbouring Censuses. 

We use a spatio-temporal algorithm that estimates Census information about each electorate, at the time of the election of interest. Since no Census occurs on the desired election year, we have to superimpose the electoral boundaries onto the neighbouring Censuses, and estimate Census characteristics in each of those years. By then linearly interpolate between these time points, we get an estimate for the election year of interest. 

To illustrate this algorithm, consider the example of the Hume electoral division in the 2013 federal election.

## Hume in the 2013 election
...
## All electorates in the 2013 election
...
# Mapping
sF_11 <- loadShapeFile("/Users/Jeremy/Documents/R/eechidna/data-raw/Shapefiles/2011_CED_shape/CED_2011_AUST.shp")
...
Then we can linearly interpolate between 2011 and 2016 to arrive at our final estimate of Census data for the electorates in place at the 2013 federal election.

```{r, eval = F}
# Linearly interpolate
abs2013 <- (2/5)*imputed_data_2016 + (3/5)*imputed_data_2011
...
```{r, echo = F}
load("/Users/Jeremy/Documents/R/eechidna/data/abs2013.rda")
head(abs2013) %>% kable
```
