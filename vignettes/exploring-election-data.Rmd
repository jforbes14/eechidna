...
`eechidna` (Exploring Election and Census Highly Informative Data Nationally for Australia) is an R package that makes it easy to look at the data from Australian Censuses and Federal Elections from 2001 to 2016.
...
fp16               | first preference votes for candidates at each electorate
tpp_pp16                | two party preferred votes for candidates at each polling place
tpp16              | two party preferred votes for candidates at each electorate
tcp_pp16                | two candidate preferred votes for candidates at each polling place
tcp16              | two condidate preferred votes for candidates at each electorate
...
load("/Users/Jeremy/Documents/R/eechidna/data/fp16.rda")
load("/Users/Jeremy/Documents/R/eechidna/data/tpp16.rda")
load("/Users/Jeremy/Documents/R/eechidna/data/tcp16.rda")
load("/Users/Jeremy/Documents/R/eechidna/data/abs2016.rda")
...
We can see that the data contains `r formatC(nrow(fp16), big.mark=",", drop0trailing=TRUE, format="f")` rows and `r ncol(fp16)` columns. Each of the six data objects can be accessed and inspected in this way. Note that some parties have slightly different names in different states. For example there is 'The Nationals' and the 'National Party'. We've simplified the data by renaming some of the parties to the varient of the name that won the most votes. 
...
census_and_election_subset <-  
  ...
  select(DivisionNm, 
         ALP_Percent,
         Population:BornElsewhere) %>% 
  select(-c(ends_with("NS"), State, EmuneratedElsewhere, InternetUse, starts_with("Age")))
...
In the plots below, we can see the Census variables that have a statistically significant relationship with the percentage of Labor votes. There seems to be a positive relationship between voting Labor and defacto relationships, postgraduate degrees and public housing tenants. Labour force participation and proportion of workers in extractive jobs (e.g. mining, gas, agriculture), and median personal income are a negatively correlated variables, meaning that increases in the proportion of workers in extractive jobs and increases in median personal income correlate with decreases in the percentage of the Labor vote in that electorate.
