...
`eechidna` (Electoral Commission Handling Intelligent Data Nationally for Australia) is an R package that makes it easy to look at the data from Australian Censuses and Federal Elections from 2001 to 2016.
...
In the `eechidna` package we have six datasets for each election, summarised here:

Data object name        | Description
----------------------- | ------------------------------------------------------------
fp_pp16                 | first preference votes for candidates at each polling place
fp_elec16               | first preference votes for candidates at each electorate
tpp_pp16                | two party preferred votes for candidates at each polling place
tpp_elec16              | two party preferred votes for candidates at each electorate
tcp_pp16                | two candidate preferred votes for candidates at each polling place
tcp_elec16              | two condidate preferred votes for candidates at each electorate

The suffix `16` corresponds with the 2016 federal election, so objects for other elections will have an equivalent suffix, e.g. `fp_elec07` contains first preference votes for candidates at each electorate in the 2007 federal election.
...
We can see that the data contains `r formatC(nrow(fp16), big.mark=",", drop0trailing=TRUE, format="f")` rows and `r ncol(fp16)` columns. Each of the four data objects can be accessed and inspected in this way. Note that some parties have slightly different names in different states. For example there is 'The Nationals' and the 'National Party'. We've simplified the data by renaming some of the parties to the varient of the name that won the most votes. 
...
total_votes_for_parties <- fp16 %>% 
  select(PartyNm, Count) %>% 
  group_by(PartyNm) %>% 
  summarise(total_votes = sum(Count, rm.na = TRUE)) %>% 
  ungroup() %>%
...
who_most_votes <- fp16 %>% 
  ...
  summarise(total_votes_for_candidate = sum(Count, rm.na = TRUE)) %>% 
...
who_most_votes_prop <- fp16 %>% 
  ...
  mutate(candidate_full_name = paste0(GivenNm, " ", Surname, " (", CandidateID, ")")) %>% 
  group_by(candidate_full_name) %>% 
  summarise(total_votes_for_candidate = sum(Percent, rm.na = TRUE)) %>% 
  arrange(desc(total_votes_for_candidate))
# inspect
who_most_votes_prop %>% 
  mutate(total_votes_for_candidate = 
...
who_most_votes_prop <-  fp16 %>% 
  ...
  dplyr::group_by(DivisionNm, candidate_full_name) %>% 
  dplyr::summarise(sum_votes = sum(Count)) %>% 
...
who_won_least_votes_prop <- 
 fp16 %>% 
   ...
   mutate(candidate_full_name = 
            paste0(GivenNm, " ", Surname, " (", CandidateID, ")")) %>% 
   group_by(DivisionNm, candidate_full_name) %>% 
   summarise(sum_votes = sum(Count)) %>% 
...
p <- fp16 %>%
  ...
  summarise(
    ...
    ProportionLabor = round(sum(Count[PartyNm == "Australian Labor Party"]) / TotalVotes, 3)) %>%
  filter(TotalVotes != 0) %>% 
...
proportions <- fp16 %>%
  filter(CandidateID != 999) %>% # exclude informal votes
  group_by(Electorate) %>%
  summarise(Prop_Labour = sum(Count[PartyNm == "Australian Labor Party"]) / sum(Count),
            Prop_Coalition = sum(
              Count[PartyNm == "Liberal"],
              Count[PartyNm == " Country Liberal Party"],
              Count[PartyNm == "The Nationals"],
              Count[PartyNm == "Country Liberals (NT)"]
              ) / sum(Count),
            Prop_Greens = sum(Count[PartyNm == "The Greens"]) / sum(Count)) 
