...
In the `eechidna` package we have two election datasets. One is called `aec2013`, and each row represents a particular canditate at a particular polling place. We can and can inspect this data like this:
...
We can see that the data contains `r formatC(nrow(aec2013), big.mark=",", drop0trailing=TRUE, format="f")` rows and `r ncol(aec2013)` columns.
