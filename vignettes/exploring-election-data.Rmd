...
all_vars <- census_and_election %>% 
  ...
  select(
         Average_Australian_Labor_Party_Percentage_in_electorate,
         Population:NotOwned) %>% 
  lm(Average_Australian_Labor_Party_Percentage_in_electorate ~ ., data = . ) %>% 
  tidy %>% 
  filter(p.value < 0.05) %>% 
  arrange(p.value)
...
census_and_election %>% 
  ...
  gather(variable, 
         value, 
         -Average_Australian_Labor_Party_Percentage_in_electorate) %>% 
  ggplot(aes(Average_Australian_Labor_Party_Percentage_in_electorate, 
                                value)) +
  geom_point() +
...
Rsqall = colwise(Rsq)(census_and_election[, census_variables])
Rsqall

names(Rsqall)[which.max(Rsqall)]

  census_and_election %>% 
    gather_(key = census_variables, value =  'variables', -'Electorate')
