...
```{r load-read-merge, message=FALSE, error = TRUE, warning=FALSE, echo=TRUE}
library(eechidna)
library(tidyverse)
...
map.winners <- fp16 %>% 
  ...
  left_join(nat_map16, by = c("DivisionNm" = "elect_div"))
```
...
```{r group, message=FALSE, error = TRUE, warning=FALSE, echo=TRUE}
# Grouping different Lib/Nats togethers
map.winners <- map.winners %>% 
  mutate(PartyNm = as.character(PartyNm),
         PartyNm = ifelse(PartyNm %in% c("LIBERAL PARTY", "NATIONAL PARTY"), 
                          "LIB/NATS COALITION", PartyNm)) %>% 
  arrange(group, order)
...
cart.winners <- fp16 %>% 
  ...
  left_join(nat_data16, by = c("DivisionNm" = "elect_div"))
