...
The function `getElectorateShapes` extracts a list from the shapefile, consisting of a `dataframe` containing coordinates of each polygon and a `dataframe` with data associated with each polygon (electorate). These can be used directly with `ggplot` graphics. Alternatively, the `loadShapeFile` function imports the shapefile as a `SpatialPolygonsDataFrame`.
