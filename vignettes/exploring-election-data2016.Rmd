---
title: "Exploring the 2016 Election data"
...
output: 
    rmarkdown::html_vignette:
        toc: true
vignette: >
  %\VignetteIndexEntry{Exploring the 2016 Election data}
...
`eechidna` (Electoral Commission Handling Intelligent Data Nationally for Australia) is an R package that makes it easy to look at the data from the 2011 Australian Census, and the 2016 Federal Election.

This vignette documents how to access the data from the 2016 Federal Election (House of Representatives (first preferences and two party preferred). We obtained these data from http://results.aec.gov.au/ . This package incorporates data that is copyright Commonwealth of Australia (Australian Electoral Commission) 2016. 
...
----------------------- | ------------------------------------------------------------
aec2016_fp              | first preference votes for candidates at each polling place
aec2016_fp_electorate   | first preference votes for candidates at each electorate
aec2016_2pp             | two party preferred votes for candidates at each polling place
aec2016_2pp_electorate  | two party preferred votes for candidates at each electorate
...
head(aec2016_fp) %>% kable
```

We can see that the data contains `r formatC(nrow(aec2016_fp), big.mark=",", drop0trailing=TRUE, format="f")` rows and `r ncol(aec2016_fp)` columns. Each of the four data objects can be accessed and inspected in this way. Note that some parties have slightly different names in different states. For example there is 'The Nationals' and the 'National Party'. We've simplified the data by renaming some of the parties to the varient of the name that won the most votes. 
...
```{r who_highest_proportion-votes, eval = FALSE}
who_most_votes_prop <- 
aec2016_fp %>% 
...
who_won_least_votes_prop <- 
 aec2016_fp %>% 
   ...
   ungroup %>% 
   left_join(aec2016_2pp_electorate %>% 
               mutate(candidate_full_name = 
            paste0(GivenNm, " ", 
                   Surname, " (", 
                   CandidateID, ")")), 
...
census_and_election <- left_join(aec2016_2pp_electorate, 
                                 abs2011,
...
A quick look at the senate results for 2016, using barchart by overall composition of the senate, and by state. 

```{r senate, echo=TRUE, tidy=TRUE, warning=FALSE, error=FALSE, message=FALSE, fig.width=5, fig.height=3}
library(readr)
senate <-
  read_csv("http://results.aec.gov.au/20499/Website/Downloads/SenateSenatorsElectedDownload-20499.csv", skip=1)
